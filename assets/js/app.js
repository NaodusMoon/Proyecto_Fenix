// --- BASE DE DATOS DE COMBOS SEMANALES CON IMÁGENES ASOCIADAS ---
        ...
        function renderCompleteMaterialCostChart() {
            ...
            const pricedItemCount = knownGroups.reduce((sum, group) => sum + group.items.length, 0);
            const auditSum = knownGroups.map((group) => `$${group.total.toLocaleString('es-CO')}`).join(' + ');
        ...
        function refineTargetMarket() {
            ...
            if (matrixCard) {
                ...
                const comparisons = [
                    { line: 'Bomba de arequipe', own: 5200, brand: 'Caibo Bakery', reference: 'Bomba rellena de chocoarequipe', rival: 8700, source: 'https://www.rappi.com.co/restaurantes/900076441-caibo-bakery', value: 'Masa frita y relleno de arequipe.' },
                    { line: 'Quesillo', own: 7000, brand: 'Caibo Bakery', reference: 'Quesillo venezolano', rival: 9500, source: 'https://www.rappi.com.co/restaurantes/900076441-caibo-bakery', value: 'Textura suave y caramelo artesanal.' },
                    { line: 'Bomba de crema pastelera', own: 5500, brand: 'Pan Casero Café', reference: 'Bomba de crema pastelera', rival: 13800, source: 'https://www.rappi.com.co/restaurantes/900284778-pan-casero-cafe', value: 'Masa suave y crema pastelera.' }
                ];
                matrixCard.classList.add('lg:col-span-2');
                matrixCard.innerHTML = `<span class="text-oro-600 font-bold text-xs uppercase tracking-wider">Matriz comparativa · V-02</span><h3 class="font-serif text-2xl md:text-3xl font-bold text-chocolate-900 mt-2">Las tres referencias ya comparadas en V-01</h3><p class="text-xs text-chocolate-600 mt-2">La matriz detalla exactamente las comparaciones comerciales previas: precio directo de Zynareth frente al precio publicado de cada referencia equivalente.</p><div class="mt-5 space-y-3">${comparisons.map((item) => { const saving = item.rival - item.own; const percent = ((saving / item.rival) * 100).toFixed(1).replace('.', ','); return `<article class="rounded-2xl border border-chocolate-100 bg-chocolate-50 p-4"><div class="grid grid-cols-1 md:grid-cols-[1.05fr_1.25fr_.9fr_.7fr] gap-4 items-center"><div><span class="text-[10px] uppercase font-bold text-oro-700">Línea comparable</span><strong class="block text-chocolate-900 mt-1">${item.line}</strong><span class="text-xs text-chocolate-600">Zynareth · $${item.own.toLocaleString('es-CO')}</span></div><div><span class="text-[10px] uppercase font-bold text-oro-700">Referencia comercial V-01</span><a href="${item.source}" target="_blank" rel="noreferrer" class="block font-bold text-chocolate-900 mt-1 underline decoration-oro-400 underline-offset-2">${item.brand} <i class="fa-solid fa-arrow-up-right-from-square text-[9px]"></i></a><span class="text-xs text-chocolate-600">${item.reference} · $${item.rival.toLocaleString('es-CO')}</span></div><div><span class="text-[10px] uppercase font-bold text-oro-700">Diferencia directa</span><strong class="block text-emerald-700 text-xl mt-1">$${saving.toLocaleString('es-CO')}</strong><span class="text-xs text-chocolate-600">menos en Zynareth</span></div><div class="rounded-xl bg-white p-3 text-center"><strong class="block text-blue-700 text-lg">${percent}%</strong><span class="text-[10px] text-chocolate-600">menor frente a referencia</span></div></div><p class="mt-3 border-t border-chocolate-100 pt-3 text-xs text-chocolate-600"><strong class="text-chocolate-800">Valor que se comunica:</strong> ${item.value}</p></article>`; }).join('')}</div><p class="mt-4 text-[11px] text-chocolate-600"><strong>Fuente y criterio:</strong> se mantienen las mismas tres referencias y precios mostrados en V-01 (consulta indicada allí: 31 de agosto de 2026). Caibo Bakery aparece dos veces porque se compararon dos líneas distintas; no se inventa un tercer negocio para completar la matriz.</p>`;
            }
        }
